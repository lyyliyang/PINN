...
for _ in range(20):
    ...
    df = pd.read_excel(loc_data("Energy efficiency/ENB2012_data.xls"))
    ...
    LL0=-tf.log(dist0.prob(label[:,0:y_dim])+1e-12)
    ...
    LL=tf.log(dist.prob(label[:,0:y_dim])/np.std(y,0)+1e-12)
