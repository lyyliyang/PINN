...
for _ in range(5):
    ...
    net3= tf.nn.relu(tf.matmul(net2, w3)+b3,name='net3')
    ...
    (grads, _) = tf.clip_by_global_norm(grads, clip_norm=20)
    ...
    for i in range(1000):
        ...
        _,l,yy,var,ls,log_p,dy=sess.run([train,loss,net4,sigma,log_sigma,LL,diff],feed_dict={s:x_train[index[:train_size]],label:y_train[index[:train_size]],tf_is_training:True})
        l_all.append(l)
        if i%5==0:
    ...
    _,l,yy,var,ls,log_p,dy=sess.run([train,loss,net4,sigma,log_sigma,LL,diff],feed_dict={s:x_train,label:y_train})
