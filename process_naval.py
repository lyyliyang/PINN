...
for _ in range(20):
    ...
    y = df[endog_columns].values-np.mean(df[endog_columns].values,axis=0)
    ...
    w1 = tf.get_variable('w1_s', [x_dim, 1600],initializer=tf.random_normal_initializer(0., .1/np.sqrt(x_dim)))
    b1 = tf.get_variable('b1', [1, 1600],initializer=tf.random_normal_initializer(0., .1/np.sqrt(x_dim)))
    w2 = tf.get_variable('w2_s', [1600, 1000],initializer=tf.random_normal_initializer(0., .1/np.sqrt(1600)))  
    b2 = tf.get_variable('b2', [1, 1000],initializer=tf.random_normal_initializer(0., .1/np.sqrt(1600)))
    w3 = tf.get_variable('w3_s', [1000, y_dim+Ncs*2+y_dim],initializer=tf.random_normal_initializer(0., .5/np.sqrt(1000)))  
    ...
    LL=tf.log(dist.prob(label[:,:y_dim])/np.std(y,0)+1e-12)
    diff=tf.squared_difference(net3[:,:y_dim], label[:,:y_dim])*np.var(y,0)
